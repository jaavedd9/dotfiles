...
(setq notmuch-binary "/usr/bin/notmuch") 
...
;;   :ensure t
;;   )
;; ref: https://bostonenginerd.com/posts/notmuch-of-a-mail-setup-part-2-notmuch-and-emacs/
;; Setup some keybindings
;; C-c m opens up Notmuch from any buffer
;;(global-set-key (kbd "C-c m") `notmuch)

'; Setup Names and Directories
(setq user-mail-address "jaavedkhan@kfupm.edu.sa"
      user-full-name "Jaaved Ali Khan")
;; stores postponed messages to the specified directory
(setq message-directory ".mail/drafts") ;

;; set sent mail directory
(setq notmuch-fcc-dirs ".mail/sent")
...
;;       (quote
;;        (
;;         (:name "unread" :query "tag:unread" :key "f" :sort-order newest-first) ;unread
...
;; (setq user-full-name "Jaaved Ali Khan")
;; (setq user-mail-adress "jaavedkhan@kfupm.edu.sa")
;; (setq mail-user-agent 'message-user-agent)
;; ;; (setq mail-specify-envelope-from t)
;; (setq mail-host-address "smtp.kfupm.edu.sa")
...
;;       mail-envelope-from 'header
;;       message-sendmail-envelope-from 'header
;;       user-mail-adress "jaavedkhan@kfupm.edu.sa")
...
(setq notmuch-mail-dir (concat dropbox-path "emacs/.mail"))
...
(progn
  ...
  (push '(:name "KFUPM"
        ...
        notmuch-saved-searches)
  (push '(:name "EDesk"
                :query "edesk@kfupm.edu.sa"
                :key "K"
                :sort-order newest-first
                )
        notmuch-saved-searches)
  (push '(:name "Code"
                :query "code@kfupm.edu.sa"
                :sort-order newest-first
                :search-type 'tree
                )
        notmuch-saved-searches)
  (push '(:name "jaavedd9"
                :query "jaavedd9@gmail.com"
                :key "G"
                :sort-order newest-first
                )
        notmuch-saved-searches)
  (push '(:name "Open Project"
                :query "ictc-projects@kfupm.edu.sa"
                :key "o"
                :sort-order newest-first
                :search-type 'tree
                )
...
(eval-after-load 'notmuch-show
  '(progn
     ...
     (define-key notmuch-search-mode-map (kbd "m")
       'vedang/notmuch-move-sender-to-social-media)
...
;; jak
(defun vedang/notmuch-move-sender-to-social-media ()
  "For the email at point, move the sender of that email to Social Media Emails.
This means:
1. All new email should be tagged =social-media=.
2. All existing email should be updated to add the tag =social-media=."
  (interactive)
  (vedang/notmuch-add-addr-to-db (vedang/notmuch-get-from)
                                 (format "%s/social_media.db" notmuch-hooks-dir))
  (vedang/notmuch-tag-by-from '("+social-media")))
...
;; 	    org-msg-recipient-names '(("jaavedkhan@kfupm.edu.sa" . "Jaaved Khan"))
